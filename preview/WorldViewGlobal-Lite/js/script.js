(() => {
  ...
  function populateContactIntent() {
    // Contact links can pre-select the right inquiry flow from CTA sections.
    if (!contactForm) {
      return;
    }
    ...
    contactForm.addEventListener("submit", (event) => {
      try {
        ...
        // Session storage can fail in private browsing or strict privacy modes.
      }
  ...
  function populateThankYou() {
    ...
    try {
      ...
      // Leave the default copy in place when storage is unavailable.
    }
  ...
  function createShootingStar() {
    if (!worldBackground || reduceMotion || !isVisible) {
      return;
    }
  ...
  function stopShootingStars() {
    if (shootingStarIntervalId) {
      ...
      shootingStarIntervalId = 0;
    }
  }

  function maybeRunShootingStars() {
    stopShootingStars();
